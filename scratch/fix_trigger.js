...
async function run() {
  try {
    console.log('Fixing block_bot_require_fullname trigger function...');
    ...
    await pool.query(`
      ...
      BEGIN
        ...
        -- RULE 3: RATE LIMIT — Max 2 entries per email per 10 seconds
        SELECT COUNT(*) INTO recent_email_count
        FROM pickle_appointment
        WHERE email = NEW.email
          AND created_at > NOW() - INTERVAL '10 seconds';

        IF recent_email_count >= 2 THEN
          RAISE EXCEPTION 'Security Block: Too many rapid bookings from this email.';
        END IF;
